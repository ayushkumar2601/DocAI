...
const Prediction: React.FC<{ user: UserProfile }> = ({ user }) => {
  const [isSubscribed] = useState(false); // mock subscription state
    ...
    <div className="space-y-10">
      <header>
        ...
        <p className="text-neutral-500 max-w-lg">
          Advanced longitudinal analysis to predict future health trajectories and preventive pathways.
        </p>
      </header>

      {/* User Info Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 opacity-70">
        <div className={`${UI_CLASSES.card} !p-6`}>
          <p className={UI_CLASSES.subheading}>Chronological Age</p>
          <p className="text-3xl font-bold">{user.age || 'N/A'}</p>
        </div>
        <div className={`${UI_CLASSES.card} !p-6`}>
          <p className={UI_CLASSES.subheading}>Patient Profile</p>
          <p className="text-3xl font-bold">{user.gender || 'N/A'}</p>
        </div>
        <div className={`${UI_CLASSES.card} !p-6`}>
          <p className={UI_CLASSES.subheading}>Primary Objective</p>
          <p className="text-xl font-bold truncate">{user.goal || 'N/A'}</p>
        </div>
      </div>

      {/* Subscription Gate */}
      <div className={`${UI_CLASSES.card} min-h-[480px] flex items-center justify-center text-center p-12`}>
        {!isSubscribed ? (
          <div className="max-w-md space-y-6 animate-reveal">
            <div className="w-16 h-16 mx-auto rounded-full bg-neutral-100 flex items-center justify-center text-2xl">
              🔒
            </div>

            <h3 className="text-xl font-bold text-neutral-900">
              Premium Health Forecast
            </h3>

            <p className="text-sm text-neutral-500 leading-relaxed">
              360° predictive health insights, early risk detection, and preventive pathways
              are available exclusively for subscribed users.
            </p>

            {!showQR ? (
              <button
                onClick={() => setShowQR(true)}
                className="mt-4 px-6 py-3 rounded-full bg-neutral-900 text-white text-sm font-bold tracking-wide hover:opacity-90 transition"
              >
                Subscribe Now
              </button>
            ) : (
              <div className="space-y-4 animate-reveal">
                ...
                <div className="mx-auto w-48 h-48 rounded-2xl overflow-hidden border border-neutral-200">
                  <img
                    src="/qr-code1.png"
                ...
                <p className="text-[11px] text-neutral-400">
                  Complete payment to unlock your personalized health forecast.
                </p>
              </div>
            )}
          </div>
